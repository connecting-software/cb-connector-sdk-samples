...
{
    public class HavingHandler : Having, IHandler
    {
        public HavingHandler(ExecutionSession session)
        {
            Session = session;
        }
        ...
        public void ExecuteInternal(IResultSetLoader loader, IExecutionContext context)
        {
            ...
            //Handles the WHERE clause in a parsed statement.
            Session.CommandInfo.Where = SqlTranslator.Instance.Translate(Arguments.Filter, context);
        }
    }
}